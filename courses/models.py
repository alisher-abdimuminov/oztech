...
NOTIFCATION_RECEIVER_TYPE = (("all", "All"), ("specific", "Specific"))
...
class Question(models.Model):
    ...
    type = models.CharField(
        max_length=100, choices=QUESTION_TYPE, verbose_name="Savol turi"
    )
...
class Answer(models.Model):
    question = models.ForeignKey(
        Question, on_delete=models.CASCADE, verbose_name="Savol"
    )
...
class Quiz(models.Model):
    ...
    questions = models.ManyToManyField(
        Question, related_name="quiz_questions", verbose_name="Savollar"
    )
    created = models.DateTimeField(auto_now_add=True)
    updated = models.DateTimeField(auto_now=True)
...
class Module(models.Model):
    ...
    required = models.ForeignKey(
        ...
        null=True,
        blank=True,
        verbose_name="Talab qilinadi",
    )
    students = models.ManyToManyField(
        User, related_name="module_students", blank=True, verbose_name="Talabalar"
    )
    finishers = models.ManyToManyField(
        User, related_name="module_finishers", blank=True, verbose_name="Bitirganlar"
    )
    ...
    def video_length(self) -> int:
        return (
            Lesson.objects.filter(module=self)
...
class Lesson(models.Model):
    ...
    next = models.ForeignKey(
        ...
        null=True,
        blank=True,
        verbose_name="Keyingi dars",
    )
    finishers = models.ManyToManyField(
        User, related_name="lesson_finishers", blank=True, verbose_name="Tugatganlar"
    )
...
class Test(models.Model):
    ...
    url = models.FileField(
        upload_to="files/lessons/tests",
        null=True,
        blank=True,
        verbose_name="Test manbaa",
    )
...
class Resource(models.Model):
    ...
    lesson = models.ForeignKey(Lesson, on_delete=models.CASCADE)
    url = models.FileField(
        upload_to="files/lessons", null=True, blank=True, verbose_name="Manbaa"
    )
...
class Notification(models.Model):
    ...
    receivers = models.ManyToManyField(
        User, related_name="notification_receivers", blank=True
    )
    readers = models.ManyToManyField(
        User, related_name="notification_readers", blank=True
    )
